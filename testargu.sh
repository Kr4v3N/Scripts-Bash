...
### testargu.sh  ###
#
#L'objet de ce challenge est de mettre en œuvre ce contrôle des arguments. Pour cela vous devez écrire un script que vous appelerez testargu.sh qui attend  2 ou 3  arguments et qui doit répondre aux spécifications suivantes :
...
# Votre script doit réaliser les vérifications sur ses arguments dans l'ordre ci-dessous. Dès qu'une des conditions n'est pas vérifiée, votre script doit afficher le message "usage : testargu.sh fichier repertoire [nombre_positif ]" sur la sortie d'erreur puis :
...
#     Terminer avec le code retour 5 si la condition sur le deuxième argument n'est pas vérifiée (la condition sur le premier argument ayant été vérifiée).
#     Terminer avec le code retour 6 s'il y a 3 arguments et si la condition sur le troisième argument n'est pas vérifiée (la condition sur les deux premiers arguments ayant été vérifiée).
